const { DEFAULT_OPTIONS, EMPTY_ALT, imageClass, imageBackgroundClass, imageWrapperClass } = require(`./constants`);
...
module.exports = (
  ...
  const findParentLinks = ({ children }) =>
    children.some(node => (node.type === `html` && !!node.value.match(/<a /)) || node.type === `link`);
  ...
  const generateImagesAndUpdateNode = async function(node, resolve, inLink, overWrites = {}) {
    ...
    const alt = isEmptyAlt ? `` : _.escape(overWrites.alt ? overWrites.alt : node.alt ? node.alt : defaultAlt);

    const title = node.title ? _.escape(node.title) : alt;

    // @mphilpot
    const loading = options.loading || 'auto';
    ...
    // @mphilpot
    const decoding = options.decoding || 'auto';
    ...
    const enabledFormatConfigs = formatConfigs.filter(({ propertyName }) => options[propertyName]);

    if (enabledFormatConfigs.length) {
      const sourcesHtmlPromises = enabledFormatConfigs.map(async ({ format, propertyName }) => {
      ...
      const sourcesHtml = (await Promise.all(sourcesHtmlPromises)).filter(sourceHtml => sourceHtml !== null);
    ...
    const wrapperStyle =
      typeof options.wrapperStyle === `function` ? options.wrapperStyle(fluidResult) : options.wrapperStyle;

    // Construct new image node w/ aspect ratio placeholder
    const imageCaption = options.showCaptions && (await getImageCaption(node, overWrites));
    ...
    const bgImage = removeBgImage ? `` : ` background-image: url('${placeholderImageData}'); background-size: cover;`;
      ...
      ({ node, inLink }) =>
        new Promise(resolve => {
          ...
          if (isRelativeUrl(node.url) && supportedExtensions[fileType]) {
            return generateImagesAndUpdateNode(node, resolve, inLink, overWrites).then(rawHTML => {
              if (rawHTML) {
